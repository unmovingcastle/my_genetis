...
def dataWrite(numChildren,varVector, freq_coeffs, freqVector):
  print(varVector)
...
parser.add_argument("NPOP", help="# of individual per generation", type=int);
...
if(g.start_cont=="start"):
  ...
  ''' calling the dataWrite function here !!!'''
  dataWrite(g.NPOP, varOutput, freq_coeffs, freqVector);
  meanTotal = 0.0;
  for i in range(g.NPOP):
    meanTotal += varOutput[i][0][1];
  meanForGridSize = meanTotal / g.NPOP

elif (g.start_cont == "cont"):
  print("continuing")