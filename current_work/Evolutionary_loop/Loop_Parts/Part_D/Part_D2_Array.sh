...
# as long as we're not done with AraSim (increase $nFiles each time AraSim success)
while [ "$nFiles" != "$totPop" ] 
do
	...
	do
		# echo $file
		# If there is no file, then it will look at the literal name given in the for loop (gen_*).
		...
		then
			...
			if   grep "segmentation violation" ../AraSim_Errors/${current_file}.error \
				|| grep "DATA_LIKE_OUTPUT" ../AraSim_Errors/${current_file}.error \
				|| grep "CANCELLED" ../AraSim_Errors/${current_file}.error \
				|| grep "please rerun" ../AraSim_Errors/${current_file}.error 
			then
				...
				cd $WorkingDir
				output_name=$WorkingDir/Run_Outputs/$RunName/AraSim_Outputs/${current_file}.output
				error_name=$WorkingDir/Run_Outputs/$RunName/AraSim_Errors/${current_file}.error
				sbatch \
				 --export=ALL,gen=$gen,num=${current_individual},WorkingDir=$WorkingDir,RunName=$RunName,Seeds=${current_seed},AraSimDir=$AraSimExec \
				 --job-name=AraSimCall_AraSeed_$gen_${current_individual}_${current_seed}.run \
			   --output=$output_name --error=$error_name Batch_Jobs/AraSimCall_AraSeed.sh
				...
				then
					echo "This individual succeeded" > \
					 ../AraSimConfirmed/${current_individual}_${current_seed}_confirmation.txt
				fi
			fi
		fi
	done
...
# if [ $gen -eq 10000 ]
# then
# 	cp $WorkingDir/Antenna_Performance_Metric/AraOut_ActualBicone.txt\
# 	 $WorkingDir/Run_Outputs/$RunName/AraOut_ActualBicone.txt
# fi