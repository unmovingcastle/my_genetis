...
# when we do that, we end up making the files only readable; we should just overwrite them
...
# When we use the sed command, anything can be the delimiter between each of the arguments;
# usually, we use /, but since there are / in the thing we are trying to substitute in 
# ($WorkingDir), we need to use a different delimiter that doesn't appear there
