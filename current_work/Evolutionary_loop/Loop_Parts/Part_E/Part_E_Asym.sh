...
#
# 1. Takes AraSim data and concatenates each file name into one string that is then used to 
#    generate fitness scores 
# 2. Then gensData.py extracts useful information from generationDNA.csv and fitnessScores.csv,
#    and writes to maxFitnessScores.csv and runData.csv
# 3. Copies each .uan file from the Antenna_Performance_Metric folder and moves to 
#    Run_Outputs/$RunName folder
#
###############################################################################################
...
cp ARA_Bicone_Data/AraOut_Actual_Bicone_Fixed_Polarity_2.9M_NNU.txt \
 Run_Outputs/$RunName/Generation_Data/Generation_${gen}/AraOut_ActualBicone.txt
...
do
	...
	# Had .txt but that's also in the fitnessfunction executable. 
done
...
then
	...
	./fitnessFunction.exe $NPOP $Seeds $ScaleFactor\
	 $WorkingDir/Generation_Data/generationDNA.csv $GeoFactor $InputFiles 
	# Here's where we add the flags for the generation
	...
	then
		# Compile the separation distance fitnessFunction.
		g++ -std=c++11 fitnessFunction_ARA_Sep.cpp -o fitnessFunction_Sep.exe
		./fitnessFunction_Sep.exe $NPOP $Seeds $ScaleFactor\
		 $WorkingDir/Generation_Data/generationDNA.csv $GeoFactor $InputFiles 
		# Here's where we add the flags for the generation

	else
		g++ -std=c++11 fitnessFunction_ARA_Asym.cpp -o fitnessFunction_asym.exe
		./fitnessFunction_asym.exe $NPOP $Seeds $ScaleFactor\
		 $WorkingDir/Generation_Data/generationDNA.csv $GeoFactor $InputFiles 
		# Here's where we add the flags for the generation
	fi
fi

cp fitnessScores.csv \
 $WorkingDir/Run_Outputs/$RunName/Generation_Data/Generation_${gen}/${gen}_fitnessScores.csv
mv fitnessScores.csv $WorkingDir/Generation_Data/

cp vEffectives.csv \
 $WorkingDir/Run_Outputs/$RunName/Generation_Data/Generation_${gen}/${gen}_vEffectives.csv
mv vEffectives.csv $WorkingDir/Generation_Data/

cp errorBars.csv \
 $WorkingDir/Run_Outputs/$RunName/Generation_Data/Generation_${gen}/${gen}_errorBars.csv
...
# python $WorkingDir/Antenna_Performance_Metric/polar_plotter.py \
#  $WorkingDir/Run_Outputs/$RunName/${gen}_Gain_Plots $RunName 14 $NPOP $gen
# Plotting software for Veff(for each individual) vs Generation
python Veff_Plotting.py $WorkingDir/Run_Outputs/$RunName \
 $WorkingDir/Run_Outputs/$RunName $gen $NPOP $Seeds 
...
# if [ $NSECTIONS -eq 1 ]
# then
# 	python LRTPlot.py $WorkingDir/Generation_Data \
# 	 $WorkingDir/Run_Outputs/$RunName $next_gen $NPOP $GeoFactor
# else
# 	# Let's consider whether or not we're evolving the separation distance
# 	if [ $SEPARATION -eq 0 ]
# 	then
# 		python LRTPlot2.0.py $WorkingDir/Generation_Data \
# 		 $WorkingDir/Run_Outputs/$RunName $next_gen $NPOP $GeoFactor $NSECTIONS	
# 	else
# 		python LRTSPlot.py $WorkingDir/Generation_Data \ 
# 		 $WorkingDir/Run_Outputs/$RunName $next_gen $NPOP $GeoFactor $NSECTIONS
# 	fi
# fi

# Run 3D Plots of L,R,T vs Fitness
# python 3DLength.py "$WorkingDir" "$WorkingDir"/Run_Outputs/$RunName \
#  $next_gen $NPOP $GeoFactor $NSECTIONS
# python 3DRadius.py "$WorkingDir" "$WorkingDir"/Run_Outputs/$RunName \
#  $next_gen $NPOP $GeoFactor $NSECTIONS
# python 3DTheta.py "$WorkingDir" "$WorkingDir"/Run_Outputs/$RunName \
#  $next_gen $NPOP $GeoFactor $NSECTIONS
...
do
	...
	cp AraOut_${gen}_${i}_${j}.txt \
	 $WorkingDir/Run_Outputs/$RunName/AraOut/AraOut_${gen}/AraOut_${gen}_${i}_${j}.txt
	done
done 
