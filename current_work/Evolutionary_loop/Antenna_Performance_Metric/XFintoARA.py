...
# Date:
#   2/21/19
#   Modified from the old XFintoARA.py from Jorge Torres, Nov 2017.
# Goal: 
#   Translate XF output into readable AraSim input. 
# Comments:	
#   You need to have all your XF output files in the same folder (see uanLoc), with 
...
freqVals = [83.33, 100.00, 116.67, 133.33, 150.00, 166.67, 183.34, 200.00, 216.67, 233.34, 
250.00, 266.67, 283.34, 300.00, 316.67, 333.34, 350.00, 366.67, 383.34, 400.01, 416.67, 
433.34, 450.01, 466.67, 483.34, 500.01, 516.68, 533.34, 550.01, 566.68, 583.34, 600.01,
616.68, 633.34, 650.01, 666.68, 683.35, 700.01, 716.68, 733.35, 750.01, 766.68, 783.35, 
800.01, 816.68, 833.35, 850.02, 866.68, 883.35, 900.02, 916.68, 933.35, 950.02, 966.68, 
983.35, 1000.00, 1016.70, 1033.40, 1050.00, 1066.70]
numFreq = 60
...
uanLoc = "/fs/ess/PAS1960/BiconeEvolutionOSC/BiconeEvolution/current_antenna_evo_build/"\
         "XF_Loop/Evolutionary_Loop/Antenna_Performance_Metric/"
...
########  DEFINITIONS #########################################################################
# Read the (indiv)freqNum.uan file
def readFile(indiv, freqNum):
  n = 37
  m = 73
  # uanName = uanLoc+str(indiv)+"_"+str(freqNum)+".uan"
  # changed from g.indiv + 1 to just indiv 
  uanName = f'{g.WorkingDir}/Run_Outputs/{g.RunName}/uan_files/{g.gen}_{indiv}_{freqNum}.uan' 
  f = open(uanName, "r")

  for a in range(18):
     f.readline()
      
  mat = [["0" for x in range(n)] for y in range(m)]  # ie. m rows n cols // Jason 0612,23
  for i in range(n):  
    for j in range(m):
      ...
      linearGainNum = "%.2f" % 10 ** (linearGainNum / 10) # %'s are for string interpolation
...
parser.add_argument("NPOP", help="How many antennas are being simulated each generation?",
                    type=int)
...
parser.add_argument("RunName", help="Run Name directory where everything will be stored",
                    type=str)
